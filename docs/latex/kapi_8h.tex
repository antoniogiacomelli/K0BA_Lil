...
\hline
{\em time\+Slice} & Time-\/slice for the task (\texorpdfstring{$>$}{>}1 to be set, 0 or 1 considers the systick period) \\
\hline
{\em priority} & Task priority \\
\hline
{\em run\+To\+Complet} & Cooperative only function, to use in deferred handlers, servers -\/ once dispatched it is never preempted unless it blocks or yields. \\
\hline
\end{DoxyParams}
\begin{DoxyReturn}{Returns}
