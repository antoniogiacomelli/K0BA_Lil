...
static inline void kListNodeDel_(K_LISTNODE *nodePtr)
{
	...
	/*for tcbs, this establishes a rule:
	 * a RUNNING task has NULL prev/next pointers */
	nodePtr->prevPtr = NULL;
	nodePtr->nextPtr = NULL;
}

/* init the list by setting head pointers and size */
K_ERR kListInit(K_LIST *const self, STRING listName)
{
	...
	self->size = 0U;
	__DMB(); /*guarantee all data is updated before going*/
	return K_SUCCESS;
}
...
/* remove head node from the list - dequeue */
