...
static K_ERR kInitTcb_(const TASKENTRY taskFuncPtr, UINT32 *const stackAddrPtr,
		const UINT32 stackSize);

static K_ERR kInitStack_(UINT32 *const stackAddrPtr, const UINT32 stackSize,
		const TASKENTRY taskFuncPtr)
{
...
K_ERR kCreateTask(const TASKENTRY taskFuncPtr, const char *taskName,
